...
] + [f"r{i}deplos" for i in range(3, 17)] + [f"r{i}deptir" for i in range(3, 17)] + \
  [f"r{i}depnoap" for i in range(3, 17)] + [f"r{i}dephun" for i in range(3, 17)] + \
  [f"r{i}depsle" for i in range(3, 17)] + [f"r{i}depnit" for i in range(3, 17)] + \
  [f"r{i}depcon" for i in range(3, 17)] + [f"r{i}depdown" for i in range(3, 17)] + \
...
def calculate_ad_mci(row):
    ...
    if row['r10demene2'] == "1.yes-has/had cond" or row['r10alzhee2'] == "1.yes-has/had cond":
        return 'AD'
...
wave_columns = [
  ...
  [f"r{i}depyr" for i in range(3, 17)] + [f"r{i}deplos" for i in range(3, 17)] + \
  [f"r{i}deptir" for i in range(3, 17)] + [f"r{i}depnoap" for i in range(3, 17)] + \
  [f"r{i}dephun" for i in range(3, 17)] + [f"r{i}depsle" for i in range(3, 17)] + \
  [f"r{i}depnit" for i in range(3, 17)] + [f"r{i}depcon" for i in range(3, 17)] + \
  [f"r{i}depdown" for i in range(3, 17)] + [f"r{i}deptho" for i in range(3, 17)] + \
  [f"r{w}demene2" for w in range(10, 17)] + [f"r{w}alzhee2" for w in range(10, 17)] + \
  [f"r{w}memrye2" for w in range(4, 10)] + [f"r{w}demens" for w in range(11, 17)]
