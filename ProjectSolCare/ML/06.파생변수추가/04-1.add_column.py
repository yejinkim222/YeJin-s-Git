...
# 데이터 로드
df = pd.read_csv(
    "C:/workspace/Project01/data/hrs/selected_data/11.ml_start/07.AD_train_with_features.csv"
)
...
df["has_db_and_hibpe"] = (
    (df["years_until_db"] != -1) & (df["years_until_hibpe"] != -1)
).astype(int)
df["has_any_chronic"] = (
    (df["years_until_db"] != -1)
    | (df["years_until_hibpe"] != -1)
    | (df["has_hibpe"] == 1)
).astype(int)
