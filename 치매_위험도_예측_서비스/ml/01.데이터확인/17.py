...
# 2️⃣ 연도 리스트 생성 (1995부터 2025까지 홀수년)
years = list(range(1995, 2026, 2))  # 홀수년

# 3️⃣ cog 데이터에서 연도별 컬럼만 추출
...
# 7️⃣ NaN 값을 'missing'으로 처리 (Categorical 변수의 경우 'missing'을 카테고리로 추가)
...
# 8️⃣ NaN 값을 'missing'으로 처리 (object 타입 컬럼은 object로 변환 후 처리)
