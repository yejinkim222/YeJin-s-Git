...
model = lgb.LGBMRegressor(
    max_depth=6,                # 🔧 기존 4 → 6으로 증가 (트리 깊이 확보)
    learning_rate=0.03,
    n_estimators=1000,
    num_leaves=40,              # 🔧 기존 31 → 40으로 증가 (리프 노드 수 확대)
    min_split_gain=1e-3,        # 🔧 0.0 → 1e-3로 변경하여 불필요한 경고 제거
    min_child_samples=5,        # 🔧 기존 10 → 5로 감소 (리프에 더 적은 샘플도 허용)
    random_state=42
)
